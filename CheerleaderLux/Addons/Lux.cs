...
namespace CheerleaderLux.Addons
{
    ...
    public class Lux : Extensions.Statics
    {
        ...
        private static void EisAlive(GameObject sender, EventArgs args)
        {
            ...
            if (sender.Name == "Lux_Base_E_mis.troy" && !sender.IsEnemy)
            {
                Lux_E = sender;
        ...
        private static void EisGone(GameObject sender, EventArgs args)
        {
            ...
            if (sender.Name == "Lux_Base_E_tar_nova.troy" && !sender.IsEnemy)
            {
                Lux_E = null;
        ...
        private static void RefreshObjects(EventArgs args)
        {
            ...
            var qcollision = Q.GetCollision(player.Position.To2D(), new List<Vector2> { Q.GetPrediction(target).CastPosition.To2D() });
        ...
        public static void SpellCast(Obj_AI_Hero target, float range, Spell spellslot, bool collision, byte count, bool objectcheck, HitChance Hitchance)
        {
            ...
            if (collision)
            {
                Q.SetSkillshot(0.25f, 200f, 1200f, false, SkillshotType.SkillshotLine);
            }
        ...
        private static void FarmMethod()
        {
            ...
            if (efarmpos.MinionsHit >=  eminions &&
                E.IsReady() && Config.Item("laneclear.E").GetValue<bool>() && Environment.TickCount - Q.LastCastAttemptT > 450)
            ...
            if (qfarmpos.MinionsHit >= qminions &&
                Q.IsReady() && Config.Item("laneclear.Q").GetValue<bool>() && Environment.TickCount - E.LastCastAttemptT > 450)
        ...
        public static void Routine()
        {
            ...
            if (Config.Item("advanced.R.aoe").GetValue<bool>() && R.IsReady() && !E.IsReady() && target.IsValidTarget(E.Range + E.Width))
            {
                R.CastIfWillHit(target, hitcount);
            }
            ...
            if (Config.Item("combo.Q").GetValue<bool>() 
                && Environment.TickCount - E.LastCastAttemptT > 400 && Environment.TickCount - R.LastCastAttemptT > 800)
                SpellCast(target, Q.Range, Q, true, 1, false, HitChance.Medium);
            ...
            if (thp < IgniteDamage(target) + rdmg + aa && rooted && Ignite.IsReady() && R.IsReady())
            {
                player.Spellbook.CastSpell(Ignite, target);
                Printchat("Ignite casted");
            }

            if (thp < IgniteDamage(target) + rdmg + aa && Ignite.IsReady() && R.IsReady())
            {
                player.Spellbook.CastSpell(Ignite, target);
                Printchat("Ignite casted");
            }
        ...
        {
            if (target == null || !target.IsValid || !R.IsReady()) return;
            ...
            if (Environment.TickCount - E.LastCastAttemptT < 100 || Environment.TickCount - Q.LastCastAttemptT < 800 && !rooted)
                return;
